# 👨‍💻 Franklin Gonçalves (Fk.Dev)

🚀 Desenvolvedor Full Stack em formação, focado em construir aplicações web completas, integrando backend, frontend e banco de dados.

Atualmente evoluindo projetos práticos com **Python, APIs e integração web**, com objetivo futuro de atuar também em **DevOps e Cloud**.

---

## 🧠 Sobre mim

* 🔁 Transição de carreira para tecnologia
* 💻 Foco atual: **Full Stack (Backend + Frontend)**
* ⚙️ Próximo passo: **DevOps & Cloud**
* 📚 Aprendizado contínuo com projetos reais
* 🎯 Objetivo: atuar como Desenvolvedor Full Stack

---

## 🛠️ Stack Tecnológica

### 🔙 Backend

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge\&logo=python\&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge\&logo=flask\&logoColor=white)

### 🌐 Frontend

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge\&logo=javascript\&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge\&logo=html5\&logoColor=white)

### ⚙️ Ferramentas

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge\&logo=git\&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge\&logo=github)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge\&logo=docker\&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-000000?style=for-the-badge\&logo=linux\&logoColor=white)

---

## 🚀 Estudos Atuais & Roadmap

* 🔹 Desenvolvimento Full Stack (APIs + Frontend)
* 🔹 Integração de sistemas (Python + JavaScript)
* 🔹 Preparação para DevOps (Docker + CI/CD)

---

## 📈 Projetos em Destaque

---

### 🔹 Olá Mundo

📂 https://github.com/franklingoncalvess/Ola-Mundo
🌐 Deploy: *(em breve)*

🧠 **Descrição técnica:**

* Projeto inicial focado em lógica de programação
* Estrutura básica em Python
* Base para evolução de projetos mais complexos

📷 Preview:
![Preview](https://via.placeholder.com/600x300)

---

### 🔹 Calculadora Full Stack

📂 https://github.com/franklingoncalvess/calculadora-python-estudos
🌐 Deploy: *(em breve)*

🧠 **Descrição técnica:**

* Backend em Python com Flask (API REST)
* Frontend em HTML + JavaScript
* Comunicação via Fetch API
* Separação entre backend e frontend

📷 Preview:
![Preview](https://via.placeholder.com/600x300)

---

### 🔹 Integração Node/Python

📂 https://github.com/franklingoncalvess/projeto-integracao-py-node
🌐 Deploy: *(em breve)*

🧠 **Descrição técnica:**

* Integração entre serviços Node.js e Python
* Comunicação entre diferentes tecnologias
* Estrutura modular de aplicação

📷 Preview:
![Preview](https://via.placeholder.com/600x300)

---

## 📊 Estatísticas

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=franklingoncalvess\&show_icons=true\&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=franklingoncalvess\&layout=compact\&theme=tokyonight)

---

## 🌐 Contato

* 💼 LinkedIn:https://www.linkedin.com/in/franklingoncalvess
* 📧 Email: [franklingoncalvess@gmail.com](mailto:franklingoncalvess@gmail.com)
* 🌐 Portfólio: https://franklingoncalvess.github.io/portfolio-pessoal

---

> 💡 *"A evolução é constante, um commit de cada vez."*
